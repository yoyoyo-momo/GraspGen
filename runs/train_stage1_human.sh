#!/usr/bin/env bash
# Retrain GraspGen generator after accumulating more grasps.
#
# Run this after accumulate_mug_grasps.sh finishes. It:
#   1. Counts grasps in mug_combined.yaml
#   2. Converts YAML → training JSON
#   3. Launches a fresh training run (wipes the old log dir)
#
# Usage:
#   bash runs/retrain_after_accumulation.sh [--resume]
...
# Handle-grasp dataset (honest successes on full-size mug handle)
COMBINED_YAML="${DATASET_DIR}/grasp_sim_data/ThreeFigV2_USDZ/mug_handle_human_dataset.yaml"
...
LOG_DIR="/home/j300/GraspGen/runs/trifinger_mug_handle_human_gen"
...
if [ ! -f "${COMBINED_YAML}" ]; then
    echo "ERROR: ${COMBINED_YAML} not found. Has accumulate_mug_grasps.sh finished?"
    exit 1
fi
