#!/usr/bin/env bash
# Retrain GraspGen generator on the flanking-only (single-archetype) dataset.
#
# The 1099-grasp run mixed two distinct A_1/B_1 archetypes (658 narrow/
# flanking + 441 widened/free-crossed) into one training set. Eval showed
# the model's predicted root poses miss the handle badly (median 54.6mm
# from the nearest handle point; only 9.6% within a plausible 20mm
# standoff), and geometric repair of its own predictions could not recover
# genuine holds even when restricted to the closest poses. Testing whether
# forcing a single, tighter grasp-shape mode (drop the free/crossed
# archetype) lets the model learn a more precise pose distribution with
# the same ~658-grasp scale, before assuming more data/scale is the fix.
#
# Usage:
...
COMBINED_YAML="${DATASET_DIR}/grasp_sim_data/ThreeFigV2_USDZ_1_a_0.01_black_blue_J002_mod2_fixed/mug_handle_scaled_final.yaml"
...
LOG_DIR="/home/j300/GraspGen/runs/trifinger_mug_handle_gen_flanking658"
CHECKPOINT="$LOG_DIR/last.pth"
CACHE_DIR="/tmp/graspgen_train_cache_trifinger_flanking658"
